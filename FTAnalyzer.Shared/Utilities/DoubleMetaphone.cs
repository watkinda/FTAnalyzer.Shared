...
namespace FTAnalyzer
{
  ...
  public class DoubleMetaphone
  {
    ...
    private static readonly IDictionary<string, DoubleMetaphone> metaphoneCache = new Dictionary<string, DoubleMetaphone>();

    public static DoubleMetaphone GetDoubleMetaphone(string word)
    {
      if (string.IsNullOrEmpty(word))
        return null;

      if (metaphoneCache.ContainsKey(word))
        return metaphoneCache[word];

      DoubleMetaphone meta = new DoubleMetaphone(word);
      metaphoneCache.Add(word, meta);
      return meta;
    }

    public static string GetPrimaryMetaphone(string word)
    {
      DoubleMetaphone meta = GetDoubleMetaphone(word);
      if (meta == null)
        return string.Empty;
